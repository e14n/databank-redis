...
DiskDatabank.prototype.disconnect = function(onCompletion) {
    var bank = this;
    if (bank.mktmp) {
        DiskDatabank.wipeDir(bank.dir, onCompletion);
    } else {
        onCompletion(null);
    }
};
...
DiskDatabank.wipeDir = function(dirname, callback) {
    ...
    Step(
		...
		fs.stat(path.join(dirname, relnames[i]), group());
            }
        },
        function(err, stats) {
            var i, group = this.group();
            ...
            for (i = 0; i < stats.length; i++) {
                if (stats[i].isDirectory()) {
                    DiskDatabank.wipeDir(path.join(dirname, relnames[i]), group());
                } else {
                    fs.unlink(path.join(dirname, relnames[i]), group());
                }
            }
        },
        function(err) {
            if (err) throw err;
            fs.rmdir(dirname, this);
        },
        callback
    );
};
